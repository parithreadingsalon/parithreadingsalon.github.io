var BASE_URL = document.getElementById('publicPath').value + '/';
...
var MapUtils = {
  Icons: {
    spritesheets: {
      baseUrl: BASE_URL + 'integration/assets/img/',
...
var Map = function(mapName){
  if (typeof(Map.maplist) === 'undefined')
      Map.maplist = {};
  Map.maplist[mapName] = this;
  var mapEntity = this;
  ...
  Map.get = function(name){
      if (typeof(Map.maplist[name]) === 'undefined')
          return null;
      return Map.maplist[name];
  };
  ...
  this.addMarkerByPosition = function (marker, textInfo, showValue, zoom, pointer, pointerColor) {
    ...
    var mk = new google.maps.Marker({
      ...
      map: this.map,
      icon: MapUtils.Icons.getIcon(pointer, pointerColor)
    });
    ...
    if (this.markers.length > 1) {
      ...
      // Si un seul marker => panTo normal
      this.center = latLng;
...
google.maps.event.addDomListener(window, 'load', function(){
	var Mp = new Map('map_BLOCK_LTM77MSWWK');
	var gmap_map_BLOCK_LTM77MSWWK = Mp.initialize({
	    'item' : 'gmap_map_BLOCK_LTM77MSWWK',
	    'zoom' : 15,
	    'mapType' : 'ROADMAP',
	    'background' : 'colors'
	});
	gmap_map_BLOCK_LTM77MSWWK.addAddress({"name":"Pari's Threading Salon","address":"25206 Northern Blvd, Little Neck, NY 11362, USA","city":"Little Neck","country":"","zip":"11362","rawAddress":"","website":"","email":"","phone":""},"25206 Northern Blvd, Little Neck, NY 11362, USA",{"lng":-73.7353654,"lat":40.7709318}, 'thin', 'black', true);
});
